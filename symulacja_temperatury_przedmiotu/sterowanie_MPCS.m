...
conditions.To = -10*ones(1, length(conditions.times));
conditions.SP = plant.pp_T(2)*ones(1, length(conditions.times));
conditions.SP(1000:3000) = 8;
conditions.SP(4500:6200) = 24;
...
%% Nadążanie + zaklócenia
conditions.times = 1:1:8000;  
ampl = 10;                                                  %max increase of disturbance power in W 
dPz = cumsum(ampl*randn(1, length(conditions.times)));      %non-measured disturbance at t
Pz = dPz + plant.pp_Pz;
Pz(Pz < 50) = 50;                                           %bounds keep dist between 50 W and 300 W
Pz(Pz > 300) = 300;
